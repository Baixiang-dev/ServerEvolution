 ...
 * 多线程 单Reactor 模型
 *
 * 在单 Reactor 模型的基础上，把业务处理投递到线程池，避免业务计算阻塞IO的处理
 */
#include <dirent.h>
#include <iostream>
#include <map>
...
public:
    ...
    void Loop()
    {
        while (!quit_)
        {
            ...
            {
                thread_pool_.enqueue([chan]() { chan->HandleEvent(); });
            }
            afterLoopTask_();
        }
    }
...
{
public:
    Server(std::string addr, int port, std::string static_dir = "web_root", int pool_size = 4)
        ...
        , running_(false)
    {
        ...
        event_loop_ = make_unique<EventLoop>(pool_size);
    }
...
int main(int argc, char* argv[])
{
    ...
    Server server(address, port, "web_root", pool_size);
    if (!server.Start())
    {
